...
In diesem Handout wird die korrekte Verwendung der erstellten MediaPipe-Pipeline erläutert.
Beispielvideos finden sich unter dem Ordner Markerlose-Bewegungserfassung\//Videos.
In dem Beispiel wird ein einfaches Arm-Modell verwendet, welches auf 3 Markern basiert
(Schulter, Ellbogen und Handgelenk). Das verwendete OpenSim Modell ist das im
GitHub-Repository befindliche Markerlose-Bewegungserfassung\//OpenSim\//models\//arm\_modell.osim.
...
die im Video nach rechts schaut. In Abbildung~\ref{fig:example_orientations}
finden sich Beispiele mit der jeweils notwendigen Rotation.
\begin{figure}[H]
    \centering
...
Für die Ursprungsverschiebung muss ein Marker gewählt werden, der als Anker für die
Verschiebung genommmen wird. Tragen Sie den Namen dieses Markers
in der Variable \texttt{anchor\_marker} ein und übergeben Sie die absolute Position dieses Markers im
OpenSim Modell in der Variable \texttt{anchor\_vector}.
