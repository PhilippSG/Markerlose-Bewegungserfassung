...
\begin{itemize}
    ...
    \item \texttt{frame\_range}: Dieser Parameter wurde explizit auf \texttt{'all'} gesetzt. 
    \textit{Begründung:} Zwar bietet die Einstellung \texttt{'auto'} theoretisch die Möglichkeit, den Eintrittszeitpunkt von Personen automatisch zu erkennen, in unseren praktischen Versuchen erwies sich diese Funktion jedoch als unzuverlässig. Um sicherzustellen, dass keine relevanten Daten verloren gehen und die gesamte Sequenz simuliert wird, wurde daher die Verarbeitung aller Frames erzwungen.
\end{itemize}

\paragraph{[pose]}
\begin{itemize}
    \item \texttt{pose\_model}: Als Modell wurde \texttt{RTMPose} (in der Variante \texttt{body\_with\_feet}) gewählt. 
    \textit{Hinweis zur Modellwahl:} In Voruntersuchungen wurde auch das Framework \textbf{MediaPipe} evaluiert. Dieses lieferte jedoch signifikant ungenauere Ergebnisse bei der 3D-Triangulierung, da die Konsistenz der Keypoints zwischen den Kameraperspektiven unzureichend war. Daher wurde es zugunsten von RTMPose verworfen.
    
    \item \texttt{mode}: Die Einstellung \texttt{'performance'} bietet den besten Kompromiss aus Genauigkeit und Rechengeschwindigkeit.
...
\begin{itemize}
    ...
    \item \texttt{method}: Es wird die Methode \texttt{'board'} (Schachbrett) verwendet. Die Alternative \texttt{'scene'} (Nutzung statischer Hintergrundpunkte) ist mathematisch komplexer und lieferte in unseren Tests weniger zuverlässige Ergebnisse.
    
    \item \textbf{Physikalische Parameter:} Entscheidend sind die korrekte Angabe der \texttt{checkerboard\_\-square\_\-size} (z.\,B. 20\,mm) und der \texttt{corners\_\-nb} (Anzahl innerer Ecken), da diese als metrische Referenz für die 3D-Umrechnung dienen.
...
\begin{itemize}
    ...
    \item Punkte unterhalb des in der \texttt{Config.toml} definierten Schwellenwerts (\texttt{likelihood\_\-threshold}) werden ignoriert, um Ausreißer zu minimieren.
\end{itemize}
